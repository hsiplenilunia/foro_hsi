...
const GalleryIsland: React.FC<GalleryIslandProps> = ({ imageItems }) => {
  ...
  return (
    <div className="w-full md:max-w-[1100px] mx-auto h-auto"style={{ minHeight: '20vh' }}>
      <div
        ref={galleryRef}
        className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-1 overflow-visible"
        style={{ maxHeight: '70vh' }}
      >
        {imageItems.slice(0, visibleImages).map((img, idx) => (
          <div
            key={img.url}
            className="flex relative group cursor-pointer items-center justify-center max-w-[220px] h-[200px] min-h-[150px] max-h-[200px]"
